...
class ChessActionClient
{
  protected:
    actionlib::SimpleActionClient<chess_msgs::StepAction> ac_;
    
  public:
    ChessActionClient(std::string name);
    ~ChessActionClient();
...
ChessActionClient::ChessActionClient(std::string name) :
  ac_(name, true)
{
  ROS_INFO("Waiting for chess action server to start...");
  ac_.waitForServer();
  ROS_INFO("Chess action server started, sending goal.");
}
ChessActionClient::~ChessActionClient()
{
}

void ChessActionClient::doStep()
{   
    ...
    // send goal
    ac_.sendGoal(goal, boost::bind(&ChessActionClient::doneCB, this, _1, _2),       boost::bind(&ChessActionClient::activeCB, this), boost::bind(&ChessActionClient::feedbackCB, this, _1));
}

void ChessActionClient::doneCB(const actionlib::SimpleClientGoalState& state,
                const chess_msgs::StepResultConstPtr& result)
{
...
void ChessActionClient::activeCB()
{
  ROS_INFO("Chess active.");
}

void ChessActionClient::feedbackCB(const chess_msgs::StepFeedbackConstPtr& feedback)
{
...
{
  ros::init(argc, argv, "chess_action_client", ros::init_options::AnonymousName);
  ...
  ChessActionClient chess_ac(action_name);
  chess_ac.doStep();

  ROS_INFO("chess_action_client main.");
  ros::shutdown();
  // ros::spin();
  return 0;
}