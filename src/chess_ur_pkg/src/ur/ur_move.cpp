...
bool UrMove::pick(chess_msgs::Operation &op)
{       
    ROS_INFO("\tbegin pick...");
    ...
    // 设置目标位姿
    geometry_msgs::Pose pose;
    pose.position.x = op.pick_x;
    pose.position.y = op.pick_y;
    pose.position.z = position_z;
    pose.orientation.x = op.pick_type == 0 ? far_o_x : near_o_x;
    pose.orientation.y = op.pick_type == 0 ? far_o_y : near_o_y;
    pose.orientation.z = op.pick_type == 0 ? far_o_z : near_o_z;
    pose.orientation.w = op.pick_type == 0 ? far_o_w : near_o_w;
